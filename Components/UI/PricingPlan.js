  ...
  return (
    <div className="flex flex-col md:flex-row justify-center gap-6 p-6  h-auto">
      {plans.map((plan) => (
        <div
          key={plan.name}
          className="bg-white shadow-xl rounded-2xl w-80 border-2 border-transparent hover:border-blue-500 transition-all"
        >
          <div className="bg-blue-600 text-white text-center py-4 rounded-t-2xl">
            <h2 S className="text-xl font-bold ">
              {plan.name}
            </h2>
            <p className="text-sm gradient-border rounded-2xl">
              {plan.duration}
            </p>
          ...
          <ul className="py-3 space-y-3">
            {plan.features.map((feature, idx) => (
              ...
              >
                ...
                <span className="text-[#656565] text-base">{feature.text}</span>
              </li>
            ))}
          </ul>
